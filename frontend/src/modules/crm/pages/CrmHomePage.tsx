    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-[var(--color-text-primary)] tracking-tight">
            {t('modulePlaceholders.crm.title', { defaultValue: 'CRM Overview' })}
          </h1>
          <p className="text-[var(--color-text-secondary)]">
            {t('modulePlaceholders.crm.subtitle', { defaultValue: 'Manage your customer relationships and leads.' })}
          </p>
        </div>
        <button className="bg-primary-600 text-white px-5 py-2 rounded-lg hover:bg-primary-700 transition shadow-md shadow-primary-500/10 active:scale-[0.98]">
          {t('modulePlaceholders.crm.newLead', { defaultValue: 'New Lead' })}
        </button>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatsCard icon={<Users />} label={t('modulePlaceholders.crm.stats.totalCustomers', { defaultValue: 'Total Customers' })} value="0" />
        <StatsCard icon={<Phone />} label={t('modulePlaceholders.crm.stats.activeLeads', { defaultValue: 'Active Leads' })} value="0" />
        <StatsCard icon={<Mail />} label={t('modulePlaceholders.crm.stats.pendingEmails', { defaultValue: 'Pending Emails' })} value="0" />
        <StatsCard icon={<Calendar />} label={t('modulePlaceholders.crm.stats.meetings', { defaultValue: 'Meetings' })} value="0" />
      </div>
      ...
      <div className="bg-warning-50/30 dark:bg-warning-900/10 border border-warning-200/50 dark:border-warning-900/30 rounded-xl p-12 text-center shadow-inner transition-all">
        ...
        <h3 className="text-xl font-bold text-warning-800 dark:text-warning-400">
          {t('modulePlaceholders.common.underDevelopment', { defaultValue: 'Module Under Development' })}
        </h3>
        <p className="text-warning-700 dark:text-warning-500 mt-2 font-medium">
          {t('modulePlaceholders.crm.message', { defaultValue: 'The full CRM module is currently under development. You have successfully activated the license!' })}
        </p>
      </div>
    </div>
  );
