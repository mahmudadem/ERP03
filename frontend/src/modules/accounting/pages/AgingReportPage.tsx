        ...
        <table className="min-w-full text-sm">
          <thead>
            <tr className="bg-gray-50">
              <th className="p-2 text-left">{t('aging.account')}</th>
              {data?.buckets.map((b, i) => (
                <th key={b} className="p-2 text-right">{b}</th>
              ))}
              <th className="p-2 text-right">{t('aging.total')}</th>
            </tr>
          </thead>
          <tbody>
                ...
                {expanded === acc.accountId && acc.entries && (
                  <tr className="bg-gray-50">
                    <td colSpan={(data?.buckets.length || 0) + 2} className="p-2">
                      <div className="text-xs font-semibold mb-1">{t('aging.transactions')}</div>
                      <table className="w-full text-xs">
                        <thead>
                          <tr>
                            <th className="text-left py-1">{t('aging.date')}</th>
                            <th className="text-left py-1">{t('aging.description')}</th>
                            <th className="text-right py-1">{t('aging.amount')}</th>
                            <th className="text-right py-1">{t('aging.days')}</th>
                          </tr>
                        </thead>
                        <tbody>
                          {acc.entries.map((e) => (
          ...
          {data && (
            <tfoot>
              <tr className="bg-gray-50 font-semibold">
                <td className="p-2">{t('aging.totals')}</td>
                {totalsRow.map((t, idx) => (
