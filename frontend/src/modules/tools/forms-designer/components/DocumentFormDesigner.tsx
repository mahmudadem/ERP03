/**
 * Document Form Designer Manager
 * 
 * ⚠️ PURE UI COMPONENT
 * 
 * Displays a list of document form/template configurations and opens the designer.
...
interface Props { // Renamed from DocumentFormDesignerProps as per the snippet
  ...
  onExit?: () => void;
  onDocumentSaved?: (document: DocumentFormConfig, isEdit: boolean) => void;
...
export const DocumentFormDesigner: React.FC<Props> = (props) => {
  ...
  // Reverted to original state management for now, as useDocumentForms is not defined.
  ...
  const handleExportSingle = (form: DocumentFormConfig) => {
    // Export as single-element array for consistency dealing with import logic
    const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify([form], null, 2));
    const downloadAnchorNode = document.createElement('a');
    downloadAnchorNode.setAttribute("href", dataStr);
    downloadAnchorNode.setAttribute("download", `document_form_${form.id}.json`);
    document.body.appendChild(downloadAnchorNode); 
    downloadAnchorNode.click();
    downloadAnchorNode.remove();
  };

  const isProtected = (form: DocumentFormConfig) => {
    return form.isSystemDefault || form.isLocked || (form as any).isSystemGenerated || (form as any).isDefault;
  };

  const handleEdit = (form: DocumentFormConfig) => {
    if (isProtected(form)) {
      setWarningModal({
        isOpen: true,
        title: 'System Default Document',
        message: 'This is a system default document that cannot be edited or deleted directly to preserve system integrity.',
  ...
  const handleDelete = async (id: string) => {
    ...
    if (form && isProtected(form)) {
      setWarningModal({
        isOpen: true,
        title: 'Cannot Delete System Document',
        message: 'This is a system default document that is protected from deletion to maintain system integrity.',
        suggestion: 'If you need a different configuration, please clone this document and customize the copy instead.'
      });
      return;
    }
  ...
  const handleExport = () => {
    ...
    const downloadAnchorNode = document.createElement('a');
    downloadAnchorNode.setAttribute("href", dataStr);
    downloadAnchorNode.setAttribute("download", "document_forms_export.json");
  ...
  const handleSave = (config: DocumentFormConfig) => {
