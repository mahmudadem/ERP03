    ...
    <>
      ...
      >
        {activeTab === 'provider' && (
          <>
            {ai.loading ? (
              <div className="flex items-center justify-center py-16 text-sm text-gray-500 bg-white border border-gray-200 rounded-lg p-6">
              ...
              <div className="space-y-8">
                ...
                <SettingsSection
                  title={t('settings.activationTitle', 'AI Assistant Activation')}
                  description={t('settings.activationDesc', 'Enable the assistant and configure floating launcher and model verification policies.')}
                ...
                >
                  ...
                  <div className="flex items-center justify-between p-4 bg-gray-50 border border-gray-200 rounded-lg">
                    ...
                    <ToggleSwitch
                      ...
                      onChange={(checked) => canManage && ai.setIsEnabled(checked)}
                      disabled={!canManage}
                    />
                  </div>
                  ...
                  <div className="flex items-center justify-between p-4 bg-sky-50 border border-sky-200 rounded-lg">
                    ...
                    <ToggleSwitch
                      checked={ai.showFloatingAssistant}
                      onChange={(checked) => canManage && ai.setShowFloatingAssistant(checked)}
                      disabled={!canManage}
                    />
                  </div>
                  ...
                  <div className="flex items-center justify-between p-4 bg-amber-50 border border-amber-200 rounded-lg">
                    ...
                    <ToggleSwitch
                      checked={ai.allowUnverifiedModels}
                      onChange={(checked) => canManage && ai.setAllowUnverifiedModels(checked)}
                      disabled={!canManage}
                    />
                  </div>
                </SettingsSection>
                ...
                <div className={ai.isEnabled ? 'space-y-8' : 'opacity-40 pointer-events-none select-none space-y-8'}>
                  {/* 2. Runtime Mode & Provider Setup */}
                  <SettingsSection
                    title={t('settings.runtimeModeTitle', 'Runtime & Provider Setup')}
                    description={t('settings.runtimeModeDesc', 'Choose a connectivity mode and configure the specific model provider.')}
                    onSave={canManage ? ai.handleSave : () => {}}
                    disabled={!ai.hasChanges || ai.saving}
                    saving={ai.saving}
                    hideSaveButton={true}
                  ...
                  {ai.runtimeMode !== 'DISABLED' && (
                    <SettingsSection
                      title={t('settings.certificationSectionTitle', 'Model Verification & Registry')}
                      description={t('settings.certificationSectionDesc', 'Register, verify compatibility, and certify custom BYOK models.')}
                      onSave={canManage ? ai.handleSave : () => {}}
                      disabled={!ai.hasChanges || ai.saving}
                      saving={ai.saving}
                      hideSaveButton={true}
                  ...
                  {/* 4. Advanced AI Constraints */}
                  {ai.runtimeMode !== 'DISABLED' && (
                    <SettingsSection
                      title={t('settings.advancedTitle', 'Advanced AI Constraints')}
                      description={t('settings.advancedDesc', 'Configure daily budgets, conversation memory settings, and token limits.')}
                      onSave={canManage ? ai.handleSave : () => {}}
                      disabled={!ai.hasChanges || ai.saving}
                      saving={ai.saving}
                      hideSaveButton={true}
                  ...
                  {ai.runtimeMode !== 'DISABLED' && (
                    <SettingsSection
                      title={t('settings.diagnosticsTitle', 'Connection Diagnostics')}
                      description={t('settings.diagnosticsDesc', 'Perform connectivity health tests to verify your provider connection is operational.')}
                      onSave={canManage ? ai.handleSave : () => {}}
                      disabled={!ai.hasChanges || ai.saving}
                      saving={ai.saving}
                      hideSaveButton={true}
