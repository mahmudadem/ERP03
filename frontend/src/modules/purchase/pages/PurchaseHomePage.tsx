    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-[var(--color-text-primary)] tracking-tight">
            {t('modulePlaceholders.purchase.title', { defaultValue: 'Purchasing Overview' })}
          </h1>
          <p className="text-[var(--color-text-secondary)]">
            {t('modulePlaceholders.purchase.subtitle', { defaultValue: 'Manage vendors, purchase orders, and procurement.' })}
          </p>
        </div>
        <button className="bg-primary-600 text-white px-5 py-2 rounded-lg hover:bg-primary-700 transition shadow-md shadow-primary-500/10 active:scale-[0.98]">
          {t('modulePlaceholders.purchase.newOrder', { defaultValue: 'New Order' })}
        </button>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatsCard icon={<ShoppingBag />} label={t('modulePlaceholders.purchase.stats.openOrders', { defaultValue: 'Open Orders' })} value="0" />
        <StatsCard icon={<Truck />} label={t('modulePlaceholders.purchase.stats.inTransit', { defaultValue: 'In Transit' })} value="0" />
        <StatsCard icon={<Receipt />} label={t('modulePlaceholders.purchase.stats.billsToPay', { defaultValue: 'Bills to Pay' })} value="0" />
        <StatsCard icon={<FileText />} label={t('modulePlaceholders.purchase.stats.rfqs', { defaultValue: 'RFQs' })} value="0" />
      </div>
      ...
      <div className="bg-[var(--color-bg-secondary)] border border-[var(--color-border)] rounded-xl p-12 text-center shadow-inner transition-colors">
        ...
        <p className="text-[var(--color-text-secondary)] mt-2 font-medium">
          {t('modulePlaceholders.purchase.message', { defaultValue: 'The Purchase module is active but we are still finalizing the UI. Check back soon!' })}
        </p>
      </div>
    </div>
  );
