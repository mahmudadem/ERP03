...
const STATUS_OPTIONS: Array<{ label: string; value: PRStatus | 'ALL' }> = [
  { label: 'All', value: 'ALL' },
...
const PurchaseReturnsListPage: React.FC = () => {
  ...
  const load = async () => {
    try {
      ...
      const list = unwrap<PurchaseReturnDTO[]>(result);
  ...
  return (
    <div className="space-y-6 p-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Purchase Returns</h1>
          <p className="text-sm text-slate-600">Track return documents posted before or after supplier invoicing.</p>
        </div>
        <button
          type="button"
            ...
            <tbody>
              {filteredReturns.map((entry) => (
                ...
                >
                  ...
                  <td className="py-2">{entry.vendorName}</td>
