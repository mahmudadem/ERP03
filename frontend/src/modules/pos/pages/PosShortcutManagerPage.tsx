...
const PosShortcutManagerPage: React.FC<{ isWindow?: boolean }> = () => {
  ...
  const selectedGroupLabel = selectedGroupId
    ? groups.find((group) => group.id === selectedGroupId)?.label || t('shortcuts.group')
    : t('shortcuts.root');
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        errorHandler.showError(err?.message || t('shortcuts.errors.load'));
      } finally {
        setLoading(false);
      }
    };
  ...
  const createLayout = async () => {
    try {
      ...
      const layout = await posApi.createProductShortcutLayout({
        name: layoutName || t('shortcuts.defaultLayoutName'),
        scopeType: 'COMPANY',
      ...
      toast.success(t('shortcuts.layoutCreated'));
    } catch (err: any) {
      errorHandler.showError(err?.response?.data?.error?.message || err?.message || t('shortcuts.errors.createLayout'));
    } finally {
      setSaving(false);
    }
  };
  ...
  const updateLayout = async (id: string, patch: Partial<PosLayoutDTO>) => {
    try {
      ...
      toast.success(t('shortcuts.layoutUpdated'));
    } catch (err: any) {
      errorHandler.showError(err?.response?.data?.error?.message || err?.message || t('shortcuts.errors.updateLayout'));
    }
  };
  ...
  const createGroup = async () => {
    ...
    try {
      ...
      setGroupName('');
      toast.success(t('shortcuts.groupCreated'));
    } catch (err: any) {
      errorHandler.showError(err?.response?.data?.error?.message || err?.message || t('shortcuts.errors.createGroup'));
    }
  };
  ...
  const addSelectedItems = async () => {
    ...
    try {
      ...
      setSelectedItemIds([]);
      toast.success(t('shortcuts.itemsAdded'));
    } catch (err: any) {
      errorHandler.showError(err?.response?.data?.error?.message || err?.message || t('shortcuts.errors.addItems'));
    } finally {
      setSaving(false);
    }
  };
  ...
  const saveNode = async () => {
    ...
    try {
      ...
      setEditing(null);
      toast.success(t('shortcuts.shortcutUpdated'));
    } catch (err: any) {
      errorHandler.showError(err?.response?.data?.error?.message || err?.message || t('shortcuts.errors.updateShortcut'));
    }
  };
  ...
  const toggleNode = async (node: PosProductShortcutNodeDTO) => {
    try {
      ...
      toast.success(t(updated.isActive ? 'shortcuts.shortcutEnabled' : 'shortcuts.shortcutDisabled'));
    } catch (err: any) {
      errorHandler.showError(err?.response?.data?.error?.message || err?.message || t('shortcuts.errors.updateShortcut'));
    }
  };
  ...
  const deleteNode = async (node: PosProductShortcutNodeDTO) => {
    try {
      ...
      toast.success(t('shortcuts.shortcutDeleted'));
    } catch (err: any) {
      errorHandler.showError(err?.response?.data?.error?.message || err?.message || t('shortcuts.errors.deleteShortcut'));
    }
  };

  if (loading) {
    return <div className="flex h-full items-center justify-center text-sm text-slate-500">{t('shortcuts.loading')}</div>;
  }
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-4">
        <header className="flex flex-col gap-3 border-b border-slate-200 pb-4 md:flex-row md:items-end md:justify-between dark:border-[var(--color-border)]">
          ...
          <div className="flex gap-2">
            <input
              ...
              placeholder={t('shortcuts.layoutName')}
            />
        ...
        <div className="grid gap-4 lg:grid-cols-[280px_minmax(280px,1fr)_minmax(360px,1.2fr)]">
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span className={`rounded-full px-2 py-0.5 text-[11px] font-semibold ${layout.isActive ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>
                      {layout.isActive ? t('shortcuts.active') : t('shortcuts.inactive')}
                    </span>
                  </div>
                  <div className="mt-2 flex items-center gap-2 text-xs text-slate-500">
                    {layout.isDefault && <span className="inline-flex items-center gap-1 text-amber-700"><Star className="h-3 w-3" /> {t('shortcuts.default')}</span>}
            ...
            {selectedLayout && (
              <div className="mt-4 space-y-2 border-t border-slate-100 pt-3">
                <button onClick={() => updateLayout(selectedLayout.id, { isDefault: true, isActive: true })} className="flex w-full items-center justify-center gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm font-semibold text-amber-800">
                  <Star className="h-4 w-4" /> {t('shortcuts.makeDefault')}
                </button>
                <button onClick={() => updateLayout(selectedLayout.id, { isActive: !selectedLayout.isActive })} className="flex w-full items-center justify-center gap-2 rounded-lg border border-slate-200 px-3 py-2 text-sm font-semibold text-slate-700">
                  ...
                  {selectedLayout.isActive ? t('shortcuts.actions.disable') : t('shortcuts.actions.enable')}
                </button>
              </div>
            )}
          </section>
          ...
          <section className="rounded-lg border border-slate-200 bg-white p-3 dark:border-[var(--color-border)] dark:bg-[var(--color-bg-secondary)]">
            ...
            <div className="mb-3 flex gap-2">
              <input value={groupName} onChange={(e) => setGroupName(e.target.value)} className="h-10 min-w-0 flex-1 rounded-lg border border-slate-300 px-3 text-sm" placeholder={t('shortcuts.groupName')} />
              <button onClick={createGroup} disabled={!selectedLayoutId || !groupName.trim()} className="inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-3 py-2 text-sm font-semibold text-white disabled:opacity-50">
                <FolderPlus className="h-4 w-4" /> {t('shortcuts.actions.add')}
              </button>
            </div>
            <div className="space-y-2">
              <button onClick={() => setSelectedGroupId('')} className={`flex w-full items-center justify-between rounded-lg border p-3 text-left text-sm ${selectedGroupId === '' ? 'border-indigo-400 bg-indigo-50' : 'border-slate-200'}`}>
                <span className="font-semibold">{t('shortcuts.rootButtons')}</span>
              ...
              {groups.map((group) => (
                <div key={group.id} className={`rounded-lg border p-3 ${selectedGroupId === group.id ? 'border-indigo-400 bg-indigo-50' : 'border-slate-200'}`}>
                  ...
                  <div className="mt-2 flex gap-2">
                    <button onClick={() => setEditing({ id: group.id, label: group.label, sortOrder: group.sortOrder, isActive: group.isActive })} className="rounded border border-slate-200 px-2 py-1 text-xs"><Edit3 className="inline h-3 w-3" /> {t('shortcuts.actions.edit')}</button>
                    <button onClick={() => toggleNode(group)} className="rounded border border-slate-200 px-2 py-1 text-xs">{group.isActive ? t('shortcuts.actions.disable') : t('shortcuts.actions.enable')}</button>
                    <button onClick={() => deleteNode(group)} className="rounded border border-rose-200 px-2 py-1 text-xs text-rose-600"><Trash2 className="inline h-3 w-3" /> {t('shortcuts.actions.delete')}</button>
                  </div>
                </div>
              ))}
            </div>
          ...
          <section className="rounded-lg border border-slate-200 bg-white p-3 dark:border-[var(--color-border)] dark:bg-[var(--color-bg-secondary)]">
            <div className="mb-3 flex items-center justify-between gap-3">
              <div>
                ...
                <p className="text-xs text-slate-500">{t('shortcuts.bulkHint')}</p>
              </div>
              <button onClick={addSelectedItems} disabled={!selectedLayoutId || selectedItemIds.length === 0 || saving} className="inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-3 py-2 text-sm font-semibold text-white disabled:opacity-50">
                <PackagePlus className="h-4 w-4" /> {t('shortcuts.addSelected')} ({selectedItemIds.length})
              </button>
            </div>

            <input value={itemSearch} onChange={(e) => setItemSearch(e.target.value)} className="mb-3 h-10 w-full rounded-lg border border-slate-300 px-3 text-sm" placeholder={t('shortcuts.searchItems')} />
            ...
            <div className="mt-4">
              <h3 className="mb-2 text-sm font-semibold text-slate-800">{t('shortcuts.assignedItems')}</h3>
              <div className="space-y-2">
                {groupItems.length === 0 && <div className="rounded-lg border border-dashed border-slate-200 p-4 text-center text-sm text-slate-500">{t('shortcuts.noItems')}</div>}
                {groupItems.map((node) => (
                  <div key={node.id} className="flex items-center justify-between gap-3 rounded-lg border border-slate-200 p-3">
                    ...
                    <div className="flex flex-none gap-2">
                      <button onClick={() => setEditing({ id: node.id, label: node.label, sortOrder: node.sortOrder, isActive: node.isActive })} className="rounded border border-slate-200 px-2 py-1 text-xs"><Edit3 className="inline h-3 w-3" /> {t('shortcuts.actions.edit')}</button>
                      <button onClick={() => toggleNode(node)} className="rounded border border-slate-200 px-2 py-1 text-xs">{node.isActive ? t('shortcuts.actions.disable') : t('shortcuts.actions.enable')}</button>
        ...
        {editing && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4">
            <div className="w-full max-w-md rounded-lg bg-white p-4 shadow-xl">
              <h3 className="mb-3 text-base font-semibold text-slate-900">{t('shortcuts.editShortcut')}</h3>
              <div className="space-y-3">
                <label className="block text-sm">
                  <span className="mb-1 block text-xs font-medium text-slate-500">{t('shortcuts.label')}</span>
                ...
                <label className="block text-sm">
                  <span className="mb-1 block text-xs font-medium text-slate-500">{t('shortcuts.sortOrder')}</span>
              ...
              <div className="mt-4 flex justify-end gap-2">
                <button onClick={() => setEditing(null)} className="rounded-lg border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700">{t('shortcuts.actions.cancel')}</button>
                <button onClick={saveNode} className="inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white"><Save className="h-4 w-4" /> {t('shortcuts.actions.save')}</button>
              </div>
            </div>
          </div>
        )}
        ...
        {selectedLayout?.isDefault && selectedLayout?.isActive && (
          <div className="flex items-center gap-2 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-emerald-800">
            <Check className="h-4 w-4" /> {t('shortcuts.ready')}
          </div>
        )}
      </div>
    </div>
