...
const PosCashierSalesReportPage: React.FC<{ isWindow?: boolean }> = ({ isWindow }) => {
  ...
  const Content: React.FC<{ params: PosDateRangeParams; setTotalItems?: (n: number) => void }> = ({ params, setTotalItems }) => {
    ...
    useEffect(() => {
      let cancelled = false;
      setLoading(true);
      posApi.getCashierSales({ dateFrom: params.dateFrom, dateTo: params.dateTo })
          ...
          <tbody>
            {rows.map((r, i) => (
              <tr key={i} className="border-b last:border-b-0">
                <td className="py-1.5 px-2 font-mono">{r.cashierUserId}</td>
                <td className="py-1.5 px-2 text-right">{r.shiftCount}</td>
...
export default PosCashierSalesReportPage;
