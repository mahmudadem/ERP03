    ...
    <div className="space-y-6 font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white border border-[#E2E8F0] p-4 rounded-lg shadow-xs flex flex-col justify-between h-24">
          <div>
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block">قيد بانتظار الموافقة / Pending approvals</span>
            <span className="text-2xl font-mono font-black text-amber-500 mt-1 block">{pendingVouchers.length}</span>
          </div>
          <div className="text-[10px] text-slate-500 font-medium">سندات تحتاج مراجعة واعتماد مالي</div>
        </div>

        <div className="bg-white border border-[#E2E8F0] p-4 rounded-lg shadow-xs flex flex-col justify-between h-24">
          <div>
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block">معدل الموافقة / Approval Rate</span>
            <span className="text-2xl font-mono font-black text-blue-600 mt-1 block">94.2%</span>
          </div>
          <div className="text-[10px] text-slate-500 font-medium">نسبة اعتماد القيود المحاسبية هذا الشهر</div>
        </div>

        <div className="bg-white border border-[#E2E8F0] p-4 rounded-lg shadow-xs flex flex-col justify-between h-24">
          <div>
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block">زمن المعالجة / Processing Time</span>
            <span className="text-2xl font-mono font-black text-emerald-600 mt-1 block">1.8 hrs</span>
          </div>
          <div className="text-[10px] text-emerald-600 font-bold">✓ تحسن بنسبة 12% عن الشهر الماضي</div>
        </div>
      </div>
      ...
      <div className="bg-white border border-[#E2E8F0] rounded-lg p-4 shadow-xs space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <h3 className="text-xs font-bold text-slate-800 uppercase">قائمة السندات بانتظار الاعتماد المالي</h3>
          </div>
          <button
            onClick={() => refetch()}
          ...
          <table className="w-full text-right text-xs">
            <thead>
              <tr className="bg-[#F8FAFC] border-b border-[#E2E8F0] text-slate-500 font-bold">
                <th className="p-3 text-center w-36">الخيارات / Actions</th>
                <th className="p-3 text-left w-32">المبلغ / Amount</th>
                <th className="p-3">البيان والوصف / Description</th>
                <th className="p-3 w-36">رقم السند / Voucher #</th>
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={5} className="p-8 text-center text-zinc-400 font-medium">
                    لا توجد سندات معلقة بانتظار الموافقة حالياً.
                  </td>
                </tr>
              ) : (
