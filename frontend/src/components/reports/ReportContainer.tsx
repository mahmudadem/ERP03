...
export function ReportContainer<TParams>({
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 overflow-hidden font-sans">
      {/* Toolbar */}
      <div className="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between shrink-0 z-30 print:hidden shadow-sm">
        <div className="flex items-center gap-6">
          <Button variant="ghost" size="sm" onClick={handleReset} className="text-slate-500 hover:text-slate-900 p-0">
            <ArrowLeft size={20} />
          </Button>
          <div className="h-8 w-px bg-slate-200" />
          <div>
            <h1 className="text-xl font-bold text-slate-900 tracking-tight leading-none">{title}</h1>
            {subtitle && <p className="text-xs text-slate-500 mt-1 uppercase font-semibold tracking-wider font-mono">{subtitle}</p>}
          </div>
        </div>

        <div className="flex items-center gap-3">
          <div className="flex items-center bg-slate-100 rounded-md p-1 border border-slate-200">
             <button onClick={() => setDensity('comfortable')} className={`p-1.5 rounded transition-all ${density === 'comfortable' ? 'bg-white shadow-sm text-slate-950' : 'text-slate-400'}`}>
                <Maximize2 size={14} />
             </button>
             <button onClick={() => setDensity('compact')} className={`p-1.5 rounded transition-all ${density === 'compact' ? 'bg-white shadow-sm text-slate-950' : 'text-slate-400'}`}>
                <Minimize2 size={14} />
             </button>
          </div>
          ...
          {config.availableColumns && (
            <div className="relative">
              <Button variant="ghost" size="sm" onClick={() => setIsSettingsOpen(!isSettingsOpen)} className={`text-slate-600 hover:bg-slate-50 font-semibold gap-2 ${isSettingsOpen ? 'bg-slate-100' : ''}`}>
                <Settings size={16} /> <span className="hidden lg:inline">Columns</span>
              </Button>
              {isSettingsOpen && (
                <>
                  <div className="fixed inset-0 z-40" onClick={() => setIsSettingsOpen(false)} />
                  <div className="absolute right-0 mt-2 w-56 bg-white border border-slate-300 shadow-xl z-50 py-1 rounded-sm">
          ...
          <div className="flex gap-1">
            {onExportExcel && (
              <Button variant="ghost" size="sm" onClick={() => params && onExportExcel(params)} className="text-slate-600 hover:bg-slate-50 p-2"><Download size={18} /></Button>
            )}
            <Button variant="ghost" size="sm" onClick={() => exportElementToPDF('report-content', title)} className="text-slate-600 hover:bg-slate-50 p-2"><FileText size={18} /></Button>
            <Button variant="ghost" size="sm" onClick={handlePrint} className="text-slate-600 hover:bg-slate-50 p-2"><Printer size={18} /></Button>
          </div>
        </div>
      </div>
      ...
      {config.paginated && isGenerated && (
        <div className="bg-white border-t border-slate-200 px-6 py-2.5 flex items-center justify-between shrink-0 print:hidden shadow-sm">
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-3">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Page</span>
              <span className="text-[11px] font-black text-slate-900 border border-slate-200 px-2 py-0.5 rounded-sm bg-slate-50">{page} / {totalPages || 1}</span>
            </div>
            <div className="h-4 w-px bg-slate-200" />
            <div className="flex items-center gap-3">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Rows</span>
              <select value={pageSize} onChange={(e) => { setPageSize(Number(e.target.value)); setPage(1); }} className="text-[11px] border-slate-200 rounded-sm focus:ring-0 py-0.5 font-bold bg-slate-50">
            ...
            <div className="flex items-center gap-3">
               ...
               <span className="text-[11px] font-black text-slate-900">{totalItems}</span>
            </div>
          </div>
          <div className="flex items-center gap-6">
             <div className="flex items-center gap-2 pr-6 border-r border-slate-100">
                <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Ref. Date</span>
                <span className="text-[11px] font-bold text-slate-900 tabular-nums">{new Date().toLocaleDateString()}</span>
             </div>
             <div className="flex gap-px bg-slate-100 rounded-sm overflow-hidden p-0.5 border border-slate-200">
                <button disabled={page === 1} onClick={() => setPage(p => Math.max(1, p - 1))} className="bg-white px-2 py-1 text-slate-700 hover:bg-slate-50 disabled:bg-slate-50 disabled:text-slate-300 border-r border-slate-100">
                  <ChevronLeft size={14} />
                </button>
                <button disabled={page >= totalPages} onClick={() => setPage(p => Math.min(totalPages, p + 1))} className="bg-white px-2 py-1 text-slate-700 hover:bg-slate-50 disabled:bg-slate-50 disabled:text-slate-300">
      ...
      <Modal isOpen={isFilterModalOpen} onClose={() => setIsFilterModalOpen(false)} title={`Configure ${title}`}>
