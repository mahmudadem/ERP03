...
import { 
  ...
  LayoutTemplate, Monitor, Smartphone, Settings, 
  FileText, Shield, Layers, PlayCircle, MousePointerClick, Save,
  Printer, Mail, Download, Upload, FileJson, GripVertical, Trash2, X, Sliders
} from 'lucide-react';
import { 
  VoucherTypeConfig, FieldLayout, UIMode, AvailableField, 
  VoucherRule, VoucherAction, VoucherActionType, SectionLayout 
} from '../types';
...
const AVAILABLE_FIELDS: AvailableField[] = [
  ...
  { id: 'account', label: 'Account', type: 'text', sectionHint: 'BODY' }, // Single line mode
...
const STEPS = [
  ...
  { id: 2, title: 'Rules', icon: Shield },
  { id: 3, title: 'Fields', icon: Layers },
  { id: 4, title: 'Actions', icon: MousePointerClick },
  { id: 5, title: 'Visual Editor', icon: LayoutTemplate },
...
export const VoucherDesigner: React.FC<VoucherDesignerProps> = ({ initialConfig, onSave, onCancel }) => {
  // Wizard State
  ...
  const runAutoPlacement = () => {
    // Only run if empty to prevent overwriting manual layout changes
    // But for this demo, we run it when leaving step 3 to populate
  ...
  const handleStepClick = (stepId: number) => {
      // Allow navigation only to previous steps or the next logical step
      // Or just allow free navigation for better UX
  ...
  const updateSectionOrder = (sectionKey: string, newOrder: number) => {
    ...
    if (targetSectionEntry) {
      ...
      (targetVal as SectionLayout).order = (currentModeSections[sectionKey] as SectionLayout).order;
      (currentModeSections[sectionKey] as SectionLayout).order = newOrder;
  ...
  const handleDropField = (e: React.DragEvent, targetSection: string, targetRow: number, targetCol: number) => {
    ...
    const sourceFields = modeConfig.sections[sourceSection].fields;
    const fieldIndex = sourceFields.findIndex(f => f.fieldId === fieldId);
    ...
    const targetFields = modeConfig.sections[targetSection].fields;
  ...
  const startResize = (e: React.MouseEvent, section: string, fieldId: string, currentSpan: number) => {
    ...
    // Global listener for smooth dragging
  ...
  const onResizeMove = (e: MouseEvent) => {
    ...
    // Immediate Update
    const overrides = { ...config.uiModeOverrides };
    const fields = overrides[previewMode].sections[section].fields;
  ...
  const updateSelectedField = (key: keyof FieldLayout, value: any) => {
    ...
    const overrides = { ...config.uiModeOverrides };
    const fields = overrides[previewMode].sections[selectedField.section].fields;
    const field = fields.find(f => f.fieldId === selectedField.id);
    if (field) {
      // @ts-ignore
      field[key] = value;
      setConfig(prev => ({ ...prev, uiModeOverrides: overrides }));
  ...
  const moveSelectedFieldSection = (newSection: string) => {
     ...
     // Remove
     const sourceFields = modeConfig.sections[selectedField.section].fields;
     const idx = sourceFields.findIndex(f => f.fieldId === selectedField.id);
     if (idx === -1) return;
     const [field] = sourceFields.splice(idx, 1);
     
     // Add to new section (append)
     field.row = modeConfig.sections[newSection].fields.length; // primitive append logic
     field.col = 0;
     modeConfig.sections[newSection].fields.push(field);
  ...
  const renderInteractiveGrid = (sectionName: string) => {
    const layout = config.uiModeOverrides[previewMode].sections[sectionName];
               ...
               {selectedField ? (
                 <div className="space-y-6">
                    ...
                    <div>
                       ...
                       <input 
                         type="text" 
                         value={config.uiModeOverrides[previewMode].sections[selectedField.section].fields.find(f => f.fieldId === selectedField.id)?.labelOverride || ''}
                    ...
                    <div>
                       ...
                       <div className="flex items-center gap-3">
                          <input 
                            ...
                            value={config.uiModeOverrides[previewMode].sections[selectedField.section].fields.find(f => f.fieldId === selectedField.id)?.colSpan || 1}
                            onChange={(e) => updateSelectedField('colSpan', parseInt(e.target.value))}
                            className="flex-1 accent-indigo-600 bg-white"
                          />
                          <span className="text-sm font-bold w-6 text-center text-slate-900">{config.uiModeOverrides[previewMode].sections[selectedField.section].fields.find(f => f.fieldId === selectedField.id)?.colSpan}</span>
                       </div>
  ...
  const renderContent = () => {
    switch (currentStep) {
          ...
          <div className="max-w-xl mx-auto space-y-6">
             <div className="space-y-4">
                <label className="block">
                  ...
                  <input type="text" value={config.name} onChange={e => setConfig({...config, name: e.target.value})} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 border bg-white text-slate-900" />
                </label>
                <div className="grid grid-cols-2 gap-4">
                  <label className="block">
                    <span className="text-sm font-medium text-gray-700">ID Key</span>
                    <input type="text" value={config.id} onChange={e => setConfig({...config, id: e.target.value})} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 border bg-white text-slate-900" />
                  </label>
                  <label className="block">
                    <span className="text-sm font-medium text-gray-700">Prefix</span>
                    <input type="text" value={config.prefix} onChange={e => setConfig({...config, prefix: e.target.value})} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 border bg-white text-slate-900" />
                  </label>
                </div>
      ...
      case 5: // Visual Editor (Re-indexed)
        return renderVisualEditor();
      case 6: // Review (Re-indexed)
        return (
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 font-sans text-slate-800 relative">
      ...
      <div className="h-20 bg-white border-t border-gray-200 px-8 flex items-center justify-between shrink-0">
         ...
         {currentStep === 6 ? (
            <button onClick={() => onSave?.(config)} className="flex items-center gap-2 px-8 py-2.5 rounded-lg font-bold text-white bg-green-600 hover:bg-green-700"><Save size={18} /> Save & Close</button>
         ) : (
            <button onClick={handleNext} className="flex items-center gap-2 px-8 py-2.5 rounded-lg font-bold text-white bg-indigo-600 hover:bg-indigo-700">Next <ArrowRight size={18} /></button>
         )}
      </div>
    </div>
  );
