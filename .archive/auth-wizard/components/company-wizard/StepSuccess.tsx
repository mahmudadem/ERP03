...
export const StepSuccess: React.FC<WizardStepProps & { onComplete: () => void }> = ({ onComplete }) => {
  return (
    <div className="flex flex-col items-center justify-center h-full py-6 md:py-12 text-center animate-in zoom-in-95 duration-500">
      ...
      <button
        onClick={onComplete}
        className="inline-flex items-center justify-center rounded-md text-sm md:text-base font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 bg-primary text-primary-foreground hover:bg-primary/90 h-10 md:h-11 px-6 md:px-8 py-2 shadow-lg hover:shadow-xl transform transition-all active:scale-95"
      >
