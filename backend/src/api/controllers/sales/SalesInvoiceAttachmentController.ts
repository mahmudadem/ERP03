...
export class SalesInvoiceAttachmentController {
  ...
  static async list(req: Request, res: Response, next: NextFunction) {
    try {
      const companyId = SalesInvoiceAttachmentController.getCompanyId(req);
      const invoiceId = String((req as any).params.id);
      const invoice = await diContainer.salesInvoiceRepository.getById(companyId, invoiceId);
      if (!invoice) {
  ...
  static async upload(req: Request, res: Response, next: NextFunction) {
    try {
      const companyId = SalesInvoiceAttachmentController.getCompanyId(req);
      const userId = SalesInvoiceAttachmentController.getUserId(req);
      const invoiceId = String((req as any).params.id);
      ...
      const path = `companies/${companyId}/sales/invoices/${invoiceId}/attachments/${Date.now()}_${sanitizedName}`;
      ...
      await diContainer.salesInvoiceRepository.update(invoice);
