...
interface AccountDoc {
  ...
  currency: string;         // The account's operating currency
}

interface LineSeed {
  accountId: string;
  side: 'Debit' | 'Credit';
  amount: number;           // In the ACCOUNT's own currency
  currency: string;         // Account's currency
  exchangeRate: number;     // Account currency → base currency
  baseAmount: number;       // amount × exchangeRate (base currency)
}
...
async function main() {
  ...
  const accounts: AccountDoc[] = accountSnap.docs.map(d => {
  ...
  const byClass = (cls: string) => posting.filter(a => a.classification === cls);
  ...
  const addToTotal = (accId: string, d: number, c: number) => {
  ...
  for (let i = 0; i < vouchers.length; i++) {
    ...
    const firestoreLines = v.lines.map((line, idx) => ({
      ...
      side: line.side,
      amount: line.amount,              // FX currency amount
      baseAmount: line.baseAmount,      // base currency amount
    ...
    const voucherData: any = {
      ...
      type: 'journal_entry',
      voucherNo,
      date: v.date,                     // plain string YYYY-MM-DD
    ...
    if (isPosted) {
      ...
      for (let li = 0; li < v.lines.length; li++) {
        ...
        batch.set(ledgerCol.doc(ledgerId), {
          ...
          voucherId: vId,
          voucherLineId: li + 1,
          date: isoToTimestamp(v.date),  // Firestore Timestamp (TB queries need this)
          debit,                       // ← BASE currency
          credit,                      // ← BASE currency
          currency: line.currency,     // original FX currency
          amount: line.amount,         // original FX amount
          baseCurrency,
    ...
    const currInfo = v.lines.map(l =>
      `${l.side === 'Debit' ? 'Dr' : 'Cr'} ${l.baseAmount.toLocaleString()} ${baseCurrency}` +
      (l.currency !== baseCurrency ? ` (${l.amount.toLocaleString()} ${l.currency})` : '')
    ).join(' / ');
  ...
  const classifications: Record<string, { debit: number; credit: number }> = {};
  let grandDebit = 0;
  let grandCredit = 0;
  ...
  for (const acc of sortedAccounts) {
    ...
    if (!classifications[cls]) classifications[cls] = { debit: 0, credit: 0 };
  ...
  // BS expected values
  const assetNet = (classifications['ASSET']?.debit || 0) - (classifications['ASSET']?.credit || 0);
  const liabNet  = (classifications['LIABILITY']?.credit || 0) - (classifications['LIABILITY']?.debit || 0);
  const eqNet    = (classifications['EQUITY']?.credit || 0) - (classifications['EQUITY']?.debit || 0);
  const revNet   = (classifications['REVENUE']?.credit || 0) - (classifications['REVENUE']?.debit || 0);
  const expNet   = (classifications['EXPENSE']?.debit || 0) - (classifications['EXPENSE']?.credit || 0);
