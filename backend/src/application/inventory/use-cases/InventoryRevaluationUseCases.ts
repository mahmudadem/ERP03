...
export class GetInventoryRevaluationUseCase {
  ...
  async execute(id: string): Promise<InventoryRevaluation | null> {
    return this.revaluationRepo.getRevaluation(id);
  }
}
...
export class CreateInventoryRevaluationUseCase {
  ...
  async execute(input: CreateInventoryRevaluationInput): Promise<InventoryRevaluation> {
    ...
    const costingBasis: 'WAREHOUSE' | 'GLOBAL' = settings?.costingBasis === 'GLOBAL' ? 'GLOBAL' : 'WAREHOUSE';
    ...
    for (const raw of input.lines) {
      ...
      const { qtyOnHand, currentAvgCostBase, currentAvgCostCCY } = await this.resolveLevelSnapshot(
        input.companyId,
        item,
        raw.warehouseId,
        costingBasis
      ...
      const valueDeltaBase = round2(qtyOnHand * (newAvgCostBase - currentAvgCostBase));
      const valueDeltaCCY = round2(qtyOnHand * (newAvgCostCCY - currentAvgCostCCY));

      lines.push({
        itemId: item.id,
        warehouseId: costingBasis === 'WAREHOUSE' ? raw.warehouseId : undefined,
        qtyOnHand: round6(qtyOnHand),
        currentAvgCostBase: round2(currentAvgCostBase),
        currentAvgCostCCY: round2(currentAvgCostCCY),
        newAvgCostBase,
        newAvgCostCCY,
        valueDeltaBase,
        valueDeltaCCY,
...
export class PostInventoryRevaluationUseCase {
  ...
  async execute(companyId: string, revaluationId: string, userId: string): Promise<InventoryRevaluation> {
    const revaluation = await this.revaluationRepo.getRevaluation(revaluationId);
    ...
    if (shouldPostAccounting && this.accountingPostingService && !settings?.defaultInventoryRevaluationAccountId) {
      throw new Error(
    ...
    await this.transactionManager.runTransaction(async (transaction) => {
      ...
      for (const line of revaluation.lines) {
        ...
        const { currentAvgCostBase, currentAvgCostCCY, qtyOnHand } = await this.resolveLevelSnapshotInTransaction(
          transaction,
          companyId,
          item,
          line.warehouseId,
    ...
    const posted = await this.revaluationRepo.getRevaluation(revaluation.id);
    if (!posted) {
  ...
  private async applyLevelWriteInTransaction(
    ...
    for (const lvl of levels) {
      ...
      lvl.avgCostBase = roundByCurrency(newAvgCostBase, item.costCurrency);
      lvl.avgCostCCY = roundByCurrency(newAvgCostCCY, item.costCurrency);
      lvl.lastCostBase = roundByCurrency(newAvgCostBase, item.costCurrency);
      lvl.lastCostCCY = roundByCurrency(newAvgCostCCY, item.costCurrency);
    ...
    if (itemStats) {
      const newCostPoint: CostPoint = {
        base: roundByCurrency(newAvgCostBase, item.costCurrency),
        ccy: roundByCurrency(newAvgCostCCY, item.costCurrency),
        currency: item.costCurrency,
        fxRateToBase: newAvgCostBase > 0 ? round2(newAvgCostBase / newAvgCostCCY) : 1,
        asOf: new Date().toISOString().slice(0, 10),
        qty: totalQty,
        source: {
          refType: 'INVENTORY_REVALUATION',
          refId: line.warehouseId || 'GLOBAL',
          docType: 'INVENTORY_REVALUATION',
          docId: line.warehouseId || 'GLOBAL',
          docNo: line.warehouseId || 'GLOBAL',
        },
      };
  ...
  private async resolveBaseCurrency(companyId: string): Promise<string> {
    const repo = (this.accountingPostingService as any)?.['companyCurrencyRepo'];
    if (repo) {
      const baseCurrency = await repo.getBaseCurrency(companyId);
      if (baseCurrency) return String(baseCurrency).toUpperCase();
    }
    return 'USD';
  }
}
