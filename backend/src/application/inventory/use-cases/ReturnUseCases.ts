...
export class ProcessReturnUseCase {
  ...
  async execute(input: ProcessReturnInput): Promise<StockMovement> {
    ...
    if (original) {
      ...
      returnUnitCostBase = level?.avgCostBase ?? 0;
      returnUnitCostCCY = level?.avgCostCCY ?? 0;
      console.warn(
        `[Inventory][ProcessReturnUseCase] Original movement not found (${input.originalMovementId}); using current average cost fallback.`
      );
    }
