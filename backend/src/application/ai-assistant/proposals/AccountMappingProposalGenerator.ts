...
export class AccountMappingProposalGenerator extends AiProposalGenerator {
  readonly proposalType = 'accounting.accountMappingProposal' as const;
  ...
  async generate(input: ProposalGeneratorInput): Promise<ProposalGeneratorOutput> {
    ...
    const riskLevel: 'low' | 'medium' | 'high' = suggestedAccount ? 'low' : 'medium';
  ...
  private findSuggestedAccount(
    description: string,
    toolData: Record<string, unknown>,
  ): { code: string; name: string; type: string } | null {
    const accounts = (toolData as any).accounts as any[] | undefined;
    ...
    for (const [category, words] of Object.entries(keywords)) {
      if (words.some(w => lower.includes(w))) {
        ...
        if (match) return { code: match.code, name: match.name, type: match.type || '' };
      }
    }
  ...
  private findAlternatives(
    ...
    return accounts.slice(0, 5).map((a: any) => ({
      code: a.code || '',
      name: a.name || '',
      type: a.type || '',
    }));
