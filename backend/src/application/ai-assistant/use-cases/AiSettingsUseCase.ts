...
export interface GetSettingsOutput {
  config: Record<string, unknown>; // Safe JSON (no apiKey exposed)
}
...
export class AiSettingsUseCase {
   ...
   */
  async getSettings(companyId: string): Promise<GetSettingsOutput> {
   ...
   */
  async updateSettings(input: UpdateSettingsInput): Promise<GetSettingsOutput> {
    ...
    if (input.provider && !this.isValidProviderType(input.provider)) {
      throw ApiError.badRequest(`Invalid provider type: ${input.provider}. Supported: mock, openai_compatible, ollama`);
    }

    // Validate API key requirements for non-mock providers
    if (input.provider === 'openai_compatible' && !input.apiKey && !config.apiKey) {
      console.warn(
        `[AI Settings] Company ${companyId} switching to OpenAI-compatible provider without an API key. ` +
    ...
    config.updateConfig({
      ...
      showFloatingAssistant: input.showFloatingAssistant,
    });
   ...
   */
  private encryptConfig(config: AiProviderConfig): AiProviderConfig {
    if (!config.apiKey) {
    ...
    const stored = AiProviderConfig.fromJSON({
      ...config.toJSON(),
      apiKey: encrypted,
  ...
  private decryptConfig(config: AiProviderConfig): AiProviderConfig {
    ...
    try {
      ...
      const plain = AiProviderConfig.fromJSON({
        ...config.toJSON(),
        apiKey: decrypted,
        updatedAt: config.updatedAt.toISOString(),
      });
      ...
      console.warn(
        `[AI Settings] Failed to decrypt API key for company ${config.companyId}. ` +
