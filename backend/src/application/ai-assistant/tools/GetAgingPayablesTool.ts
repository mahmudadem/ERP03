 ...
 */
interface AgingPayablesSummaryDTO {
  asOfDate: string;
  type: 'AP';
  buckets: string[];
  accounts: Array<{
...
export class GetAgingPayablesTool implements AiTool {
  readonly name = 'accounting.getAgingPayables';
  readonly description = 'Get an Accounts Payable aging summary for the company. Shows payables broken down by aging buckets (Current, 1-30, 31-60, etc.). Read-only — cannot create, modify, or post anything.';
  ...
  async execute(context: ToolExecutionContext, params?: Record<string, unknown>): Promise<AiToolResult> {
    try {
      ...
      return {
        success: false,
        data: null,
        error: `Failed to retrieve AP aging summary: ${(error as Error).message}`,
