...
const makeDeps = (invoice: PurchaseInvoice, opts?: { settlementFail?: boolean }) => {
  const settings = {
    defaultPurchaseExpenseAccountId: 'EXP-1',
    defaultAPAccountId: 'AP-1',
    allowDirectInvoicing: true,
    overInvoiceTolerancePct: 0,
  };
  const invSettings = null;
  const party = { id: 'vendor-1', displayName: 'Vendor A', defaultAPAccountId: 'AP-1' };
  const baseCurrency = 'USD';
  const item = { id: 'item-1', companyId: 'cmp-1', name: 'Item 1', code: 'I1', trackInventory: false, baseUomId: 'uom-1', baseUom: 'EA', purchaseUomId: 'uom-1', purchaseUom: 'EA' };
  ...
  return {
    ...
    purchaseOrderRepo: { update: jest.fn().mockResolvedValue(undefined) },
    ...
    accountingPostingService: { postInTransaction: jest.fn().mockResolvedValue({ id: 'vch-pi-1' }) },
    ...
    voucherSequenceRepo: { getNextNumber: jest.fn().mockResolvedValue('PV-001') },
...
describe('PostPurchaseInvoiceUseCase — Settlement Modes', () => {
  it('posts with DEFERRED mode — invoice remains UNPAID', async () => {
    ...
    const result = await useCase.execute('cmp-1', invoice.id, true, makeSettlementInput(0, 'DEFERRED'));
  ...
  it('posts with CASH_FULL mode — invoice becomes PAID', async () => {
    ...
    const result = await useCase.execute('cmp-1', invoice.id, true, makeSettlementInput(100, 'CASH_FULL'));
  ...
  it('posts with MULTI mode — partial settlement marks PARTIALLY_PAID', async () => {
    ...
    const result = await useCase.execute('cmp-1', invoice.id, true, {
  ...
  it('rolls back atomically when settlement sub-step fails', async () => {
    ...
    await expect(useCase.execute('cmp-1', invoice.id, true, makeSettlementInput(100, 'CASH_FULL'))).rejects.toThrow(
