...
export class PurchaseOrder {
  ...
  constructor(props: PurchaseOrderProps) {
    if (!props.id?.trim()) throw new Error('PurchaseOrder id is required');
    if (!props.companyId?.trim()) throw new Error('PurchaseOrder companyId is required');
    if (!props.orderNumber?.trim()) throw new Error('PurchaseOrder orderNumber is required');
    if (!props.vendorId?.trim()) throw new Error('PurchaseOrder vendorId is required');
    if (!props.currency?.trim()) throw new Error('PurchaseOrder currency is required');
    if (!props.createdBy?.trim()) throw new Error('PurchaseOrder createdBy is required');
  ...
  private normalizeLine(line: PurchaseOrderLine, index: number): PurchaseOrderLine {
    if (!line.itemId?.trim()) throw new Error(`PurchaseOrder line ${index + 1}: itemId is required`);
  ...
  static fromJSON(data: any): PurchaseOrder {
