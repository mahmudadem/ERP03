 ...
 */
export class FXRevaluationStrategy implements IVoucherPostingStrategy {
  async generateLines(header: any, companyId: string, baseCurrency: string): Promise<VoucherLineEntity[]> {
    ...
    header.lines.forEach((inputLine: any, idx: number) => {
      ...
      const amountBase = roundMoney(Math.abs(deltaBase));
      ...
      lines.push(new VoucherLineEntity(
        lines.length + 1, // Unique sequential ID for each line
        inputLine.accountId,
        ...
        `FX Reval: ${inputLine.currency} balance ${inputLine.foreignBalance} @ ${inputLine.newRate}`,
    ...
    const targetAmountBase = roundMoney(Math.abs(netDeltaBase));

    lines.push(new VoucherLineEntity(
      lines.length + 1, // Unique sequential ID for the offset line
      header.targetAccountId,
      isTargetDebit ? 'Debit' : 'Credit',
      targetAmountBase,   // baseAmount
      baseCurrency,       // baseCurrency
      targetAmountBase,   // amount = baseAmount
      baseCurrency,       // currency = baseCurrency
      1.0,                // exchangeRate
      `Total FX Revaluation (Unrealized ${netDeltaBase > 0 ? 'Gain' : 'Loss'})`,
      undefined,
      { totalNetDelta: netDeltaBase, isRevaluation: true }
    ));
