import { FiscalPeriod, PeriodScheme, PeriodStatus } from '../entities/FiscalYear';

export class FiscalPeriodGenerator {
  static generate(
    startDate: string | Date, 
    endDate: string | Date, 
    scheme: PeriodScheme, 
    specialPeriodsCount: number = 0
  ): FiscalPeriod[] {
    ...
    while (currentDate <= dEnd && iterations < MAX_ITERATIONS) {
      ...
      if (scheme === PeriodScheme.MONTHLY) {
        // Add 1 month, then subtract 1 day (e.g. Jan 1 -> Feb 1 -> Jan 31)
        periodEnd = new Date(periodStart.getFullYear(), periodStart.getMonth() + 1, 0);
      } else if (scheme === PeriodScheme.QUARTERLY) {
    ...
    for (let i = 0; i < specialPeriodsCount; i++) {
       const spNo = 13 + i;
       if (spNo > 16) break; // Hard limit at 16 (P13-P16)

       const spId = `FY${fyEndYear}-P${spNo}`;
       periods.push({
         id: spId,
         name: `Special Period P${spNo}`,
         startDate: this.toIso(dEnd), // Effective date is only the LAST day
