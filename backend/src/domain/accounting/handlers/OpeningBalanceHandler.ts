...
/**
 * Opening Balance Line Input
 * 
 * User specifies account and its opening balance.
 * Balance can be debit (asset) or credit (liability/equity).
 */
export interface OpeningBalanceLineInput {
  accountId: string;
  debit: number;      // Opening balance as debit (for assets)
  credit: number;     // Opening balance as credit (for liabilities/equity)
  notes?: string;
}

/**
 * Opening Balance Voucher Input Data
 * 
 * Used for initializing the accounting system with existing balances.
 */
export interface OpeningBalanceInput {
  // Required fields
  date: string;                           // Opening date (usually fiscal year start)
  description: string;                    // Description (e.g., "Opening balances as of Jan 1, 2025")
  lines: OpeningBalanceLineInput[];       // Account balances
 ...
 */
export class OpeningBalanceHandler {
  /**
   ...
   */
  async validate(input: OpeningBalanceInput): Promise<void> {
    const errors: string[] = [];
    ...
    if (input.lines && Array.isArray(input.lines)) {
      ...
      if (Math.abs(totalDebit - totalCredit) > 0.01) {
        errors.push(
          `Opening balances not balanced: Total Debit (Assets) = ${totalDebit.toFixed(2)}, ` +
