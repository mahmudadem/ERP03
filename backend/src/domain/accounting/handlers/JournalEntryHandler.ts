 ...
 * 
 * User specifies each line with account and either debit OR credit amount.
 */
export interface JournalEntryLineInput {
  accountId: string;
  debit: number;      // Amount to debit (0 if credit)
  credit: number;     // Amount to credit (0 if debit)
  notes?: string;
  costCenterId?: string;
}

/**
 * Journal Entry Voucher Input Data
 * 
 * User provides the complete breakdown of debits and credits.
 */
export interface JournalEntryInput {
  // Required fields
  date: string;                        // ISO date (YYYY-MM-DD)
  description: string;                 // Entry description
  lines: JournalEntryLineInput[];      // User-defined lines (debits and credits)
 ...
 */
export class JournalEntryHandler {
  /**
   * Validate journal entry input
   * 
   * Checks business rules before creating lines.
   */
  async validate(input: JournalEntryInput): Promise<void> {
    const errors: string[] = [];
    ...
    if (input.lines && Array.isArray(input.lines)) {
      ...
      if (Math.abs(totalDebit - totalCredit) > 0.01) {
        errors.push(
          `Entry is not balanced: Total Debit = ${totalDebit.toFixed(2)}, ` +
