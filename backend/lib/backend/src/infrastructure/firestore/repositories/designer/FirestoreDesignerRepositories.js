...
class FirestoreVoucherTypeDefinitionRepository extends BaseFirestoreRepository_1.BaseFirestoreRepository {
    ...
    async getVoucherType(companyId, id) {
        if (companyId === FirestoreVoucherTypeDefinitionRepository.SYSTEM_COMPANY_ID) {
            ...
            if (systemDoc.exists) {
                const systemDef = this.toDomain(Object.assign(Object.assign({}, systemDoc.data()), { id: systemDoc.id }));
                try {
        ...
        for (const mod of modules) {
            ...
            if (doc.exists) {
                const definition = this.toDomain(Object.assign(Object.assign({}, doc.data()), { id: doc.id }));
                try {
    ...
    async getVoucherTypesForModule(companyId, module) {
        if (companyId === FirestoreVoucherTypeDefinitionRepository.SYSTEM_COMPANY_ID) {
            ...
            return snap.docs.map(d => this.toDomain(Object.assign(Object.assign({}, d.data()), { id: d.id })));
        }
    ...
    async getByCode(companyId, code) {
        if (companyId === FirestoreVoucherTypeDefinitionRepository.SYSTEM_COMPANY_ID) {
            ...
            if (!systemSnap.empty) {
                const sysDef = this.toDomain(Object.assign(Object.assign({}, systemSnap.docs[0].data()), { id: systemSnap.docs[0].id }));
                try {
    ...
    async getSystemTemplates() {
        ...
        return snap.docs.map(d => {
            const def = this.toDomain(Object.assign(Object.assign({}, d.data()), { id: d.id }));
            try {
                VoucherTypeDefinitionValidator_1.VoucherTypeDefinitionValidator.validate(def);
            }
            catch (error) {
