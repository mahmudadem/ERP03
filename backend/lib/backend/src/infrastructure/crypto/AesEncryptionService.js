        ...
        if (envKey) {
            try {
                ...
                if (this.key.length !== KEY_LENGTH) {
                    throw new Error(`AI_ENCRYPTION_KEY must decode to ${KEY_LENGTH} bytes, got ${this.key.length}. ` +
                ...
                if (this.isProduction) {
                    throw new Error(`AI_ENCRYPTION_KEY is invalid. In production, a valid key is required. ` +
                        `Error: ${err.message}`);
                }
                console.warn(`[AesEncryptionService] AI_ENCRYPTION_KEY is invalid. Running in passthrough mode (NO ENCRYPTION). ` +
                    `This is NOT secure for production. Error: ${err.message}`);
                this.key = null;
            ...
            console.warn('[AesEncryptionService] AI_ENCRYPTION_KEY not set. Running in passthrough mode (NO ENCRYPTION). ' +
...
//# sourceMappingURL=AesEncryptionService.js.map