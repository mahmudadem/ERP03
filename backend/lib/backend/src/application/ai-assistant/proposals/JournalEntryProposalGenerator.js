...
exports.JournalEntryProposalGenerator = void 0;
const AiProposalGenerator_1 = require("./AiProposalGenerator");
class JournalEntryProposalGenerator extends AiProposalGenerator_1.AiProposalGenerator {
    constructor() {
        super(...arguments);
        this.proposalType = 'accounting.journalEntryProposal';
    ...
    async generate(input) {
        ...
        if (missingInfo.length > 1)
            riskLevel = 'high';
        if (missingInfo.length === 0 && warnings.length === 0)
            riskLevel = 'low';
        return {
    ...
    extractAmount(message, toolData) {
        ...
        // Try tool data
        if (toolData.amount && typeof toolData.amount === 'number')
            return toolData.amount;
        if (toolData.totalAmount && typeof toolData.totalAmount === 'number')
            return toolData.totalAmount;
        return null;
    }
    extractAccount(message, side, toolData) {
        // Try to extract account from tool data
        if (side === 'debit' && toolData.debitAccount) {
            const acc = toolData.debitAccount;
            return { code: acc.code || acc.accountCode || '', name: acc.name || acc.accountName || '' };
        }
        if (side === 'credit' && toolData.creditAccount) {
            const acc = toolData.creditAccount;
            return { code: acc.code || acc.accountCode || '', name: acc.name || acc.accountName || '' };
        }
...
exports.JournalEntryProposalGenerator = JournalEntryProposalGenerator;
//# sourceMappingURL=JournalEntryProposalGenerator.js.map