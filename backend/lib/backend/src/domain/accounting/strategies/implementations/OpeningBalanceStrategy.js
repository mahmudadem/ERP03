...
exports.OpeningBalanceStrategy = void 0;
...
class OpeningBalanceStrategy {
    async generateLines(header, companyId) {
        ...
        for (const balance of header.balances) {
            ...
            balance.accountId, 'Opening Balance');
        ...
        if (Math.abs(totalDebitBalance - totalCreditBalance) > tolerance) {
            throw new Error(`Total debit balances must equal total credit balances. Total debits: ${totalDebitBalance.toFixed(2)}, Total credits: ${totalCreditBalance.toFixed(2)}`);
        }
        return lines;
    }
}
exports.OpeningBalanceStrategy = OpeningBalanceStrategy;
//# sourceMappingURL=OpeningBalanceStrategy.js.map