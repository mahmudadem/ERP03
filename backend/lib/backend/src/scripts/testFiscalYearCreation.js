...
class MockPermissionChecker extends PermissionChecker_1.PermissionChecker {
    async assertOrThrow() { return; }
}
async function main() {
    console.log('Starting Fiscal Year Creation Test...');
    // 1. Setup Dependencies
    const companyRepo = bindRepositories_1.diContainer.companyRepository;
    const fiscalYearRepo = bindRepositories_1.diContainer.fiscalYearRepository;
    const permissionChecker = new MockPermissionChecker({}); // Mock dependencies
    // 2. Define Test Data
    ...
    const listUseCase = new FiscalYearUseCases_1.ListFiscalYearsUseCase(fiscalYearRepo, permissionChecker);
    ...
    const createUseCase = new FiscalYearUseCases_1.CreateFiscalYearUseCase(fiscalYearRepo, companyRepo, permissionChecker);
    try {
        ...
        // 5. Verify Structure
        if (!fy.id)
            throw new Error('FY missing ID');
        if (fy.startDate !== `${year}-01-01`)
            throw new Error(`FY startDate mismatch: ${fy.startDate}`);
        if (fy.periods.length !== 12)
            throw new Error(`FY period count mismatch: ${fy.periods.length}`);
...
//# sourceMappingURL=testFiscalYearCreation.js.map