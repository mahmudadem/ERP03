...
**Status:** ✅ Done & CTO-audited green 2026-06-21, commit `66390c23` — PASS with caveat: the promised "Sales no longer imports inventory domain entities" architecture test was **not** added; Sales still imports `Item`/`StockLevel`/`StockMovement` for **stock-OUT orchestration + revenue resolution** (out of this task's COGS-accumulation scope). Core reject conditions (COGS totals / accumulation ownership) are clear. Follow-up logged in ACTIVE.md.
