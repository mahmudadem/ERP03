...
**Status:** Part B (backend over-payment) ✅ DONE — Part A (shared `<SettlementBlock>` UI + library type) ✅ DONE — Part C (contract doc) ✅ DONE (frozen below).

> **Part A field-type registration landed (2026-06-09, branch `feat/overpayment-credit-balance`).** The shared `<SettlementBlock>` was already built and consumed by both native SI/PI pages; the remaining gap was the Forms-Designer registration. Now done: (1) seeded a `settlement` `system_core` HEADER field in `backend/src/seeder/seedFieldLibrary.ts`, scoped via `supportedTypes` to `sales_invoice` / `purchase_invoice`; (2) added `'settlement'` to the frontend `FieldType` union + an optional `settlementContext` on `FieldDefinition` (host feeds party AR/AP, outstanding, payment configs — the controlled-component contract from Part C); (3) new `frontend/src/designer-engine/components/SettlementField.tsx` adapter bridges the renderer's single `value`/`onChange` to `SettlementBlock`'s granular `mode`/`rows` props; (4) `DynamicFieldRenderer` mounts the adapter (full-width, self-titled — no duplicate label); (5) `documentMapper.mapFieldType` maps `settlement → 'settlement'` so the designer round-trip preserves the type instead of falling to `TEXT`. Frontend + backend typecheck clean; 28 settlement/settings/designer tests green.
