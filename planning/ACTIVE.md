...
- Final v1 hosting stack is now explicitly locked in `planning/tasks/DEPLOYMENT-PLAN-SUPABASE.md`: Frontend Vercel free, Backend Railway, Database Supabase/PostgreSQL, and Firebase for login/files/notifications.
- QA blocker #1 fixed locally: SQL-mode onboarding plan selection now creates the missing SQL `User` row for a verified Firebase Auth user, covering legacy/emulator sessions imported from Firestore-era auth data. Focused test, backend typecheck, backend build, and live emulator legacy-auth select-plan probe pass.
- QA blocker #2 fixed locally: Super Admin module registry no longer shows Sales as `code_only` or POS as `implementation_failed`. SQL module registry seed now uses stable `id = code`, includes Sales, and marks POS implemented for v1. Live local SQL registry was reconciled; availability endpoint now reports Accounting, Inventory, POS, Purchase, and Sales as available.
- QA blocker #3 fixed locally: SQL-mode onboarding no longer fails company initialization with `COA Template 'standard' not found in system metadata`. `PrismaSystemMetadataRepository.getMetadata('coa_templates')` now hydrates full COA templates from `chart_of_accounts_templates` while preserving manifest metadata, so the initializer receives stable ids like `standard` plus account arrays. Focused repository regression test, backend `tsc --noEmit`, idempotent SQL seed, and live SQL metadata probe pass.
- QA blocker #4 fixed locally: SQL-mode onboarding no longer fails simple trading setup with `AR parent account must be classified as ASSET (got asset)`. COA template hydration now normalizes template account `type`/`classification` to canonical uppercase, and `PrismaAccountRepository` defensively normalizes account create/update classifications before persistence. Focused repository regression test, backend `tsc --noEmit`, and live SQL metadata probe pass.
- QA blocker #5 fixed locally: Retry after a failed starter initialization no longer leaves an unusable company-name collision. The fast onboarding controller now validates the starter template before saving the company and rolls back the company + entitlement if starter initialization fails. The old failed local `zxc` shell was removed only after confirming it had no ledger, vouchers, invoices, POS receipts, or stock movements. A later local retry created `zxc` successfully with 48 accounts, 16 voucher types, 16 voucher forms, one fiscal year, and Accounting/Inventory/Sales/Purchase initialized.
- QA blocker #6 (two-company creation) fixed locally: creating a **second** company in SQL mode failed three ways — UNIQUE on blank `taxId`, duplicate global `CompanyRole.id` / voucher-type / `FY{year}` ids, and an orphan row left by a non-atomic flow. One root class: Firestore→SQL port not reconciling Postgres UNIQUE-on-`''`, stable ids as global PKs, and lack of cross-doc transactions. Fixed `taxId` (nullable), `CompanyRole` (composite PK), voucher-type/fiscal-year/voucher-sequence ids (company-scoped), voucher-form FK, classification read-normalize + 84-row backfill, and three settings/fact repos' `companyId`+`connect` create conflict. Company creation is now atomic (compensating rollback). Added **`npm run smoke:companies`** (`backend/src/scripts/smokeTwoCompanies.ts`) — creates two companies + full module init, **posts a balanced journal on each**, verifies 15 invariants each (incl. ledger balanced), cleans up; **PASS**. Orphan "ASD co" deleted. Report: `planning/done/275-two-company-creation-bug-sweep.md`. Known follow-up: admin "delete company with transactions" hits a `voucher_lines`/`ledger_entries` → accounts RESTRICT (creation rollback unaffected).
- **NEXT:** ask owner for commit approval. Do **not** merge to `main`. (After approval, consider folding `smoke:companies` into CI and extending it to assert voucher posting on both companies.)
