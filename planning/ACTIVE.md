...
- **Where I left off:** 250h is complete and green. Tax math now lives in `application/system-core/tax/TaxEngine.ts` behind `ITaxEngine`; Sales, Purchase Invoice, and POS audited paths consume the shared calculation. Invoice-discount allocation and recoverable input-tax APIs are implemented and tested, but not silently applied to live SI/PI totals. Completion reports: [250f](./done/250f-money-core.md), [250g](./done/250g-audit-engine.md), [250h](./done/250h-tax-engine.md).
...
- **Next (Phase 3):** 250i Numbering Engine → 250j Inventory Core tidy. Hard-stop after 250j for CTO audit. Do not start Phase 4 (250k/250l) unattended.
