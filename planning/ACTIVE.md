...
- **Where I left off:** 250d is implemented and locally green, pending commit: POS **sale** completion now routes through POS-owned `PostPosSaleUseCase` using `IInventoryCore` + `IAccountingBridge`, and `CompletePosSaleUseCase` no longer constructs Sales invoice use-cases or imports Sales invoice entities. The 250d sale-path architecture guard is enabled for `CompletePosSaleUseCase` + `PostPosSaleUseCase`; the folder-wide POS→Sales ban remains skipped with TODO → 250d2 as ruled. Verification: focused POS sale + architecture tests passed (3 suites, 15 tests, 1 skipped 250d2 guard), `npm --prefix backend run typecheck` passed, and `npm --prefix backend run build` passed. Completion report: `planning/done/250d-pos-posting-entry-point.md`; architecture note: `docs/architecture/pos-independence.md`.
...
- **Next:** Commit 250d as `feat(system-core): POS posts via accounting bridge, not Sales use-cases [250d]`, then continue the unattended sequence with **250d2** (returns decoupling + folder-wide POS→Sales ban), then **250e** (approval seam); hard-stop after 250e for CTO audit.
