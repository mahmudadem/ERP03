...
**Status:** ✅ **Phase 1 complete & CTO-audited green (2026-06-21)** — POS is now independent of the Sales App (sale + return). Awaiting owner go for Phase 2. **Branch:** `feat/system-core-transformation` (worktree `D:\DEV2026\ERP03-system-core`).
...
- **Where I left off:** 250d is committed at `93561820`, 250d2 is committed at `1ff23b5f`, and 250e is complete and green on the final Phase 1 task commit. POS sale/return posting now routes through POS-owned use-cases over `IInventoryCore` + `IAccountingBridge`, and the folder-wide `backend/src/application/pos/` → Sales application/domain import ban is enabled with no skip. 250e adds `ApprovalEngine`, `ApprovalSubjectRegistry`, and `LedgerCustodyApprovalPlugin`; `SubmitVoucherUseCase` now evaluates accounting voucher gates through the engine while preserving existing Smart FA/CC metadata and behavior. Verification for 250e: focused approval + existing voucher approval/posting tests passed (5 suites, 19 tests), `npm --prefix backend run typecheck` passed, and `npm --prefix backend run build` passed. Completion reports through 250e exist in `planning/done/`; architecture notes updated in `docs/architecture/system-core.md` and `docs/architecture/pos-independence.md`.
...
- **Next (Phase 2, awaiting owner go):** 250f Money Core + 250g Audit Engine — low-risk, additive, "during V1". Codex may batch 250f→250g and hard-stop after 250g for CTO audit. Do NOT enter Phase 3/4 unattended.
