...
**Additional Apex visual reports (2026-06-05):** [175-apex-shell-rtl-flyout-positioning.md](./done/175-apex-shell-rtl-flyout-positioning.md), [176-apex-prototype-typography-restoration.md](./done/176-apex-prototype-typography-restoration.md).
...
**Apex shell RTL flyout positioning & Contrast sidebar preset visual hardening complete (2026-06-05).** Fixed coordinate alignment bugs where submenus overlapped the main sidebar in RTL mode, resolved background color discrepancies by styling spawned flyout submenus with `var(--app-sidebar-surface)`, and visually hardened active item rows, row hovers, section headers, and category icon containers to use semi-transparent white overlays in Contrast (brand colored) sidebar layouts. This avoids visual bleed, invisible text (white-on-white), and invisible active row highlights in presets like Ocean Breeze.
...
**Apex prototype typography restoration complete (2026-06-05).** Compared the downloaded prototype source against ERP03 global typography. Apex now loads Inter 400-900 and JetBrains Mono 400-800, scopes the Apex shell to those fonts, and temporarily restores root font scale to 100% while mounted so it is not shrunk by the main shell's 90% global dashboard scale. Frontend typecheck passed; production build pending after final docs. **Manual QA needed:** compare sidebar/topbar/module labels and mono metadata against the downloaded prototype. Estimate: 10-15 minutes.
...
Continue **Task 167 Apex shell production candidate migration**:

1. **Slice 3C page mounting by module** (estimated 4-6 hours total, split by module) — embed native production pages for Sales, Purchases, Inventory, Settings/RBAC, and AI subroutes that are not yet Apex-native equivalents. Start with Sales operational routes because they carry posting, audit, attachments, messaging, credit, and period-lock controls.
2. **Slice 3D feature flag and cutover readiness** (estimated 2-4 hours) — put Apex behind a tenant-shell feature flag, run role/module-bundle navigation checks, verify empty data states, and complete authenticated desktop/mobile plus Arabic RTL visual QA.
3. Resume remaining Task 132 chrome work only after the Apex candidate route strategy is clear, because the shell direction has changed from legacy-shell styling to Apex-native hardening.
