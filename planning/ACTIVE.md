...
- **Done so far (all on branch `feat/275-supabase-integration`, not merged):**
  ...
  - 275e (accounting + inventory slices) — integration harness `backend/scripts/sql-integration-275e.ts`, 11 checks, run 2× on real Postgres. **Found+fixed 4 SQL bugs** that would each have broken core SQL-mode behaviour: `PrismaAccountRepository.create` input mix; `PrismaLedgerRepository.recordForVoucher` createMany+connect; `PrismaStockLevelRepository.upsertLevel` update-only (no create); `PrismaStockMovementRepository.toDomain` NULL settlement fields broke read-back. Report: `planning/done/275e-sql-integration-tests.md`.
- **NEXT — finish Task 275e:** extend the harness to the remaining modules — **Sales** (SI), **Purchases** (PI), RBAC, Core, POS. Each adds a `flowX()` to `sql-integration-275e.ts`. Given 4 bugs in the first 2 modules, expect more. Also clear the 5×`TODO(275a-audit)` + 7×`TODO(275b-audit)` markers against the live schema.
