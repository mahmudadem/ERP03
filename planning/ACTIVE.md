...
**Status:** ✅ **Phase 3 complete & CTO-audited green (2026-06-21)** — 250h Tax, 250i Numbering, 250j Inventory tidy all PASS. Only Phase 4 (250k bridge hardening, 250l Commercial Core) + epic wrap-up remain. Do not start Phase 4 unattended. **Branch:** `feat/system-core-transformation` (worktree `D:\DEV2026\ERP03-system-core`).
...
- **Where I left off:** 250j is implemented, full-suite green, and committed. `IInventoryCore` is now canonical; Sales/Purchases-named inventory contracts are deprecated aliases only. Sales Delivery Note, Sales Invoice, and Sales Return now call inventory core for COGS account resolution and COGS bucket accumulation. Number allocation from 250i is committed at `6803b21f`. Completion reports: [250f](./done/250f-money-core.md), [250g](./done/250g-audit-engine.md), [250h](./done/250h-tax-engine.md), [250i](./done/250i-numbering-engine.md), [250j](./done/250j-inventory-core-tidy.md).
...
- **Next (Phase 4 — owner decision):** 250k Accounting Bridge hardening (low/med risk) then **250l Commercial Core** — the highest-risk task; pricing/discount/promotions/cost-margin touching posting math. **250l must be run slice-by-slice (250l-1/2/3) with golden-total checks per slice, NOT as one batch.** Owner may also choose to pause the epic here (engines POS needed are done) and resume POS feature work first. Plus epic wrap-up: write `system-core.md`/`module-boundaries.md`/`pos-independence.md`, confirm all §N tests, then merge to `main`.
