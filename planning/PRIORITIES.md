...
| Claude/Opus (CTO) | Epic 275 — Task 275e: SQL integration tests per module | 2026-06-28 | 🔶 In progress. **Accounting + Inventory slices DONE & green** (`scripts/sql-integration-275e.ts`, 11 checks, run 2×) on real Postgres. **Found+fixed 4 launch-blocking SQL bugs:** (1) `PrismaAccountRepository.create` checked/unchecked input mix; (2) `PrismaLedgerRepository.recordForVoucher` `createMany` + relation connect; (3) `PrismaStockLevelRepository.upsertLevel` update-only (couldn't create new levels); (4) `PrismaStockMovementRepository.toDomain` NULL settlement fields broke read-back. Sales/Purchases/RBAC/Core/POS remain. Report: `planning/done/275e-sql-integration-tests.md`. |
