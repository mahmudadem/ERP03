...
| Claude/Opus (CTO) | Epic 275 — Task 275e: SQL integration tests per module | 2026-06-28 | 🔶 In progress. **Accounting slice DONE & green** (`scripts/sql-integration-275e.ts`, 7 checks, run 2×): JE post→ledger→trial-balance via PostingGateway on real Postgres. **Found+fixed 2 launch-blocking SQL bugs** — `PrismaAccountRepository.create` checked/unchecked input mix, and `PrismaLedgerRepository.recordForVoucher` `createMany` with relation connect. 6 modules remain (inventory/sales/purchases/RBAC/core/POS). Report: `planning/done/275e-sql-integration-tests.md`. |
