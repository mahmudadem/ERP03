...
| Claude/Opus (CTO) | Epic 275 — Task 275e: SQL integration tests per module | 2026-06-28 | 🔶 In progress. **Accounting + Inventory + Sales + Purchases slices DONE & green** (`scripts/sql-integration-275e.ts`, 15 checks, run 2×) on real Postgres. **Found+fixed 7 launch-blocking SQL bugs:** (1) Account.create input mix; (2) Ledger createMany+connect; (3) StockLevel.upsertLevel update-only; (4) StockMovement.toDomain NULL settlement fields; (5) SI/PI create scalar+connect companyId mix; (6) SI/PI schema missing `voucherType` column; (7) PurchaseInvoiceLine schema missing 6 columns (PI never persistable). ⚠️ schema changed → `prisma db push` needed. RBAC/Core/POS remain. Report: `planning/done/275e-sql-integration-tests.md`. |
