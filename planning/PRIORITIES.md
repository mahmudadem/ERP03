...
|-------|------|---------|--------|
| Codex | Production QA fixes — Telegram export, sequential commits | 2026-06-29 | 🔄 278a committed/deploy pending; 278b verified/commit approval pending |
...
| Claude/Opus (CTO) + Codex | Epic 275 — Task 275e: SQL integration tests per module | 2026-06-28 | ✅ Done. **Accounting + Inventory + Sales + Purchases + RBAC + Core + POS slices green** (`scripts/sql-integration-275e.ts`, 25 checks, run 2×) on real Postgres. **Found+fixed 8 launch-blocking SQL bugs:** (1) Account.create input mix; (2) Ledger createMany+connect; (3) StockLevel.upsertLevel update-only; (4) StockMovement.toDomain NULL settlement fields; (5) SI/PI create scalar+connect companyId mix; (6) SI/PI schema missing `voucherType` column; (7) PurchaseInvoiceLine schema missing 6 columns; (8) VoucherSequence create company connect + raw `fiscalYearId` mix. ⚠️ schema changed earlier in 275e → `prisma db push` needed. Report: `planning/done/275e-sql-integration-tests.md`. |
| Codex | Epic 275 — Task 275e RBAC/Core/POS SQL integration slices | 2026-06-28 | ✅ Done |
| Codex | Epic 275 — 275a/275b SQL audit TODO cleanup | 2026-06-28 | ✅ Done locally on `feat/275-supabase-integration`, not committed. Zero 275a/275b TODOs remain; SQL seed clean on force-reset DB; 25-check harness and `tsc --noEmit` green. Report: `planning/done/275ab-audit-resolution.md`. |
