...
Scope note: Task 267-F is migrating source-module posters behind `IAccountingBridge` in small golden-test slices. Sales DeliveryNote, SalesInvoice, SalesReturn, Sales PaymentSync, Goods Receipt, Purchase Invoice, Purchase Return, and Purchases PaymentSync paths are now bridge-only. Inventory posting paths remain follow-up slices and must keep using golden voucher-output checks before migration.
