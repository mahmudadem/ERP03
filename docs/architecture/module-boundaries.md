...
- **FUP-3:** Sales/Purchases/Inventory posters not yet all routed behind `IAccountingBridge`. Sales DeliveryNote COGS, SalesInvoice, SalesReturn, Sales PaymentSync, Goods Receipt, Purchase Invoice, Purchase Return, Purchases PaymentSync, and Inventory Opening Stock migrated to bridge-only (Task 267-F); Inventory Stock Adjustment, Stock Transfer, and Inventory Revaluation still hold direct posting fallbacks.
