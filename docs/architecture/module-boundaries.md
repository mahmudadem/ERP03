...
- **FUP-3:** Sales/Purchases/Inventory posters not yet all routed behind `IAccountingBridge`. Sales DeliveryNote COGS, SalesInvoice, SalesReturn, Sales PaymentSync, Goods Receipt, and Purchase Invoice document vouchers migrated to bridge-only (Task 267-F); Purchase Return, Purchases PaymentSync, and Inventory still hold direct posting fallbacks.
