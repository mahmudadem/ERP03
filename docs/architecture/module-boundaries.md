...
- **FUP-3:** Sales/Purchases/Inventory posters not yet all routed behind `IAccountingBridge`. Sales DeliveryNote COGS + SalesInvoice document vouchers migrated to bridge-only (Task 267-F); SalesReturn/PaymentSync/Purchases/Inventory still hold a `SubledgerVoucherPostingService` field as fallback.
