...
- **FUP-3:** Sales/Purchases/Inventory posters not yet all routed behind `IAccountingBridge`. Sales DeliveryNote COGS, SalesInvoice, SalesReturn, Sales PaymentSync, Goods Receipt, Purchase Invoice, Purchase Return, and Purchases PaymentSync migrated to bridge-only (Task 267-F); Inventory still holds direct posting fallbacks.
