...
- **`PosTerminalPage`** is a product-grid + order-panel checkout (scanner/search/manual tiles → cart with qty steppers → totals → green Pay → React-state tender dialog driven by the enabled payment methods). It never posts directly — `previewSale` supplies the authoritative tax-inclusive quote and `completeSale` posts the POS direct sale. Items with a non-positive sale price are blocked from the cart. Removing a line opens a reason dialog and marks the line voided instead of deleting it from the sale audit trail.
- **Barcode scanner capture** is terminal-level, not search-input-only. Most retail scanners behave like keyboards and send fast keystrokes followed by Enter or Tab. The terminal captures fast key bursts while no payment/edit/approval dialog is open, resolves the scanned value through `/tenant/pos/products/search`, prefers exact `barcode` or `code` matches, and then calls the same `onAddToCart` path used by search-result clicks and shortcut tiles. This keeps POS barcode scans on the existing shared item/catalog + Commercial Core pricing path instead of adding an Inventory-only lookup or bypassing pricing/tax setup. Successful barcode and manual additions play distinct lightweight WebAudio tones and refocus the search input for the next scan.
